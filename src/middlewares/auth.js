...
const verifyCallback = (req, resolve, reject) => async (err, user, info) => {
 // console.log(err,await user);
  if(err||!user||info){return reject(new ApiError(httpStatus.UNAUTHORIZED, 'Please authenticate'));}
    req.user = user;
    //console.log(req.user,await user,"from auth user");
    resolve();
};
